...
@IBDesignable
class PaymentReceivedTableViewCell: UITableViewCell {

    var transactionViewModel: TransactionViewModel?
    var interactionDelegate: OpenInteractionsDelegate?
    ...
    func loadViewFromNib() -> UIView? {
        let nibName = "PaymentReceivedTableViewCell"
        let bundle = Bundle(for: type(of: self))
        let nib = UINib(nibName: nibName, bundle: bundle)
        return nib.instantiate(withOwner: self, options: nil).first as? UIView
    }

    @IBAction func addInteractionButtonPressed(_ sender: Any) {
        self.interactionDelegate?.reactTo(cell: self)
    }
