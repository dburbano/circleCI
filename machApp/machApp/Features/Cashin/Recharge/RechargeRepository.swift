...
class RechargeRepository: RechargeRepositoryProtocol {

    var apiService: APIServiceProtocol?
    var errorParser: RechargeErrorParser?

    required init(apiService: APIServiceProtocol?, errorParser: RechargeErrorParser?) {
    ...
    func deleteCreditCard(with token: String, onSuccess: @escaping () -> Void, onFailure: @escaping (ApiError) -> Void) {
        apiService?.request(CashService.deleteCreditCard(token: token), onSuccess: { _ in
