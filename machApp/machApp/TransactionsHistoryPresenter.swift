...
class TransactionsHistoryPresenter: TransactionsHistoryPresenterProtocol {
    ...
    private func filterTransactions() {
        ...
        guard let transactions = transactions, let transactionsFilter = transactionsFilter
            else { return }
    ...
    private func updateVariables(with historyModel: HistoryModel, appendFlag: Bool) {
        ...
        if appendFlag {
            ...
            //After new transactions are added we need to sort both arrays (transactions & filteredTransactions) again
            transactions?.sort(by: { transaction0, transaction1 in
                return transaction0.transactionDate.compare(transaction1.transactionDate) == .orderedDescending
            })
            
            filteredTransactions.sort { transaction0, transaction1 in
                return transaction0.transactionDate.compare(transaction1.transactionDate) == .orderedDescending
            }
            
        } else {
            
            //The first time data is added to the arrays (transactions & filteredTransactions) we need to sort it. 
            let sortedByDateTransactions = historyModel.array.sorted(by: { transaction0, transaction1 in
                return transaction0.transactionDate.compare(transaction1.transactionDate) == .orderedDescending })
            
            transactions = sortedByDateTransactions
            filteredTransactions = sortedByDateTransactions
        }
    }
