...
enum TooltipArrowType {
    case normal
    case tilted
}

@IBDesignable
class BaseTooltipView: UIView {
    ...
    @IBInspectable var isArrowTilted: Bool = false
    ...
    private func drawTooltip() {
        ...
        if arrowBottomLeft {
            if isArrowTilted {
                ...
                bubblePath.move(to: bottomLeft(35, 2))
                bubblePath.addLine(to: bottomLeft(25, -6))
                bubblePath.addLine(to: bottomLeft(15, 2))
            }
            bubblePath.close()
        }
